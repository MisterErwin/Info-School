...
public class Bruch {
	 ...
	 * @param pZaehler Der Zaehler des Bruchs
	 * @param pNenner Der Nenner des Bruchs
	 */
	...
	public Bruch multiplizieren(Bruch pBruch){
		...
		return new Bruch(getZaehler()*pBruch.getZaehler(), getNenner() * pBruch.getNenner());
	}
	
	/**
	 * Dividert zwei Brueche durch Multiplikation des Dividenden mit dem Kehrwehrt des Divisors
	 * @param Der Divisor
	...
	public Bruch addieren(Bruch pBruch){
		Utils.validateNotNull(pBruch);
		Bruch b1 = new Bruch(getZaehler() * pBruch.getNenner(), getNenner() * pBruch.getNenner());
		Bruch b2 = new Bruch(pBruch.getZaehler() * getNenner(), getNenner() * pBruch.getNenner());
	...
	/**
	 * Ermittelt den groessten gemeinsamen Teiler des Bruches
	...
	/**
	 * Ueberschreibung der toString Funktion eines Objects zur besseren Darstellung
